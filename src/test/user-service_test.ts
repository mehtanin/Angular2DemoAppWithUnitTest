...
describe('user service', () => {
  ...
  it('should validate registration number', inject([UserService], (service) => {
    service.pin = 12345;
  ...
  it('should greet when registration number is wrong', async(inject([UserService], (service) => {
    service.pin = 9999;
    service.getGreeting().then((greeting) => {
      expect(greeting).toEqual('Sorry, failure!');
    });
  })), 3000);

  it('should greet when registration number is right', async(inject([UserService], (service) => {
    service.pin = 2015;
    service.getGreeting().then((greeting) => {
      expect(greeting).toEqual('Sorry, failure!');
    });
...
describe('with mocked registration', () => {
  ...
  it('should greet', async(inject([UserService], (service) => {
    service.getGreeting().then((greeting) => {
      expect(greeting).toEqual('Congratulations!');
    });
  })));
});
